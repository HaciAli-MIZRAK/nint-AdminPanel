 ...
 * @category      Email
 * @version       1.0.1
 * @spreading     Natan Felles
 * @develops      Hacı Ali MIZRAK
 * Imap email Haberleşme protokolü için Class
...
class Imap
{
    ...
    /**
     * Bu function Hacı Ali MIZRAK Tarafından geliştirildi ve iyileştirildi.
    ...
    public function imap_connect($config = array())
    {
        try {
            ...
            /* Bu Kısım Hacı Ali MIZRAK Tarafından eklendi */
            else:
                $enc = '/novalidate-cert'; // Hacı Ali MIZRAK
            endif;

            /* Bu Kısım Hacı Ali MIZRAK Tarafından düzenlendi */
            if($config['host'] != NULL && $config['user'] != NULL && $config['pass'] != NULL):
    ...
    /**
     * Hacı Ali MIZRAK Tarafından düzeltildi
    ...
    } // end rename_folder($name = '', $newname = '')
	 ...
	 * @return array
	 */
	public function get_messages($number = 0, $order = 'ASC', $withbody = FALSE)
	{
	...
	protected function format_message($id = 0, $withbody = TRUE)
	{
        ...
        /**
         * bu function Hacı Ali MIZRAK tarafından düzenlenmiştir.
         * @param type $str
         */
	protected function convert_to_utf8($str = '')
	{
		if (mb_detect_encoding($str, "UTF-8, ISO-8859-9, GBK") != "UTF-8"):
